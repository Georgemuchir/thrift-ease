document.addEventListener("DOMContentLoaded", () => {
  ...
  // Wait for API to be ready before allowing sign-in
  let isAPIReady = false;
  
  function checkAPIReady() {
    if (window.ThriftEaseAPI && window.ThriftEaseAPI.isReady) {
      isAPIReady = true;
      console.log('✅ ThriftEase API is ready');
      return true;
    }
    return false;
  }
  ...
  window.addEventListener('ThriftEaseAPIReady', () => {
    isAPIReady = true;
    console.log('✅ ThriftEase API ready event received');
  });
  
  // Fallback check after 2 seconds
  setTimeout(() => {
    if (!isAPIReady) {
      console.log('⚠️ API not ready after 2 seconds, checking again...');
      checkAPIReady();
    }
  }, 2000);
